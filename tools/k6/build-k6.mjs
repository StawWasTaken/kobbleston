...
import { box, join, bind } from './geometry.mjs'
...
const shape = {
  head: join([
    box({ at: [0, (H.torsoTop + H.headTop) / 2, 0], size: [2.1, H.headTop - H.torsoTop, 2.1] }),
  ]),
  torso: join([
    box({ at: [0, (H.legTop + H.torsoTop) / 2, 0], size: [3.0, H.torsoTop - H.legTop, 1.5] }),
  ]),
  arm: (side) => join([
    // shoulder to wrist
    // The arm reaches the top of the torso so the shoulder turns on the
    // corner it shares with it. Hang it any lower and raising the arm opens
    // a gap you can see straight through.
    box({ at: [side * 1.95, (H.torsoTop + 4.9) / 2, 0], size: [0.95, H.torsoTop - 4.9, 0.95] }),
    // the hand, part of the same body part rather than a seventh one
    box({ at: [side * 1.95, 4.4, 0.05], size: [1.05, 1.0, 1.1] }),
  ]),
  leg: (side) => join([
    // hip to ankle
    box({ at: [side * 0.72, 2.45, 0], size: [1.25, 3.9, 1.25] }),
    // the foot, part of the same body part
    box({ at: [side * 0.72, 0.25, 0.22], size: [1.3, 0.5, 1.85] }),
  ]),
}
