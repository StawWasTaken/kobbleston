...
import { getSpaceById, updateSpace, uploadCommunityImage } from '@/lib/api'
...
export default function EditSpace() {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      const [emblemUrl, coverUrl] = await Promise.all([
        emblem ? uploadCommunityImage(profile.id, emblem, 'emblem') : Promise.resolve(space.data.emblem_url),
        cover ? uploadCommunityImage(profile.id, cover, 'cover') : Promise.resolve(space.data.cover_url),
      ])
  ...
  const addShot = async () => {
    ...
    setPending(true)
    try {
      const url = await uploadCommunityImage(profile.id, newShot, 'cover')
    ...
    <Page className="max-w-3xl space-y-6">
      ...
      <form onSubmit={save} className="space-y-5">
        <Card className="space-y-5 p-5 sm:p-6">
          ...
          <ImageDrop label="Emblem" file={emblem} existing={space.data.emblem_url} onChange={setEmblem}
            note="The square icon people see in lists and inside the Space." />

          <ImageDrop label="Cover" aspect="wide" file={cover} existing={space.data.cover_url} onChange={setCover}
